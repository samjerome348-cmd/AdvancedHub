...
Write-Host "Installing TradeHub..."
$currentDir = Get-Location
$msixPath = ($currentDir.Path + "\" + "TradeHub.sideload.msix")
...
if ($installProc.ExitCode -eq 0) {
	$app = Get-StartApps "TradeHub"

	# If it's an array, then we found multiple matching apps. Grab the last one.
	if ($app -is [array]) {
		Write-Host "Warning: found multiple apps installed named TradeHub. Launching best guess. If the wrong app launches, find the right one in your start menu."
		$app = $app[-1];
	}

	if ($app) {
		Write-Host "Launching TradeHub..."
